...
情境：教務處要通知各系所行政同仁參加「AI 協作新思維」研習。
原始資料：8/21 早上 9 點到 12 點半，在八甲校區國際會議廳，要辦 AI 協作新思維研習，對象是教職員，請各單位派一位同仁參加。報名到 8/15 截止，請帶筆電，聯絡人是教務處王小姐，分機 1234。
            ...
            sections: [
              {
                title: "建議操作",
                items: [
                  "先讓 ChatGPT 產出可人工檢查的信件草稿，不建議一開始就要求直接寄出。",
                  "若帳號已連接 Gmail 或可在對話中使用 @Gmail，請先要求建立草稿，並保留人工確認收件人、主旨、附件與敏感資訊的步驟。",
                  "若沒有 Gmail 連接工具，就把主旨、內文與附件提醒複製到 Gmail。"
                ]
              },
              {
                title: "追加指令範例",
                items: [
                  "請把以上內容改成 Gmail 草稿格式，包含：收件人、主旨、內文、附件提醒與寄出前查核清單。若可使用 @Gmail，請只建立草稿，不要直接寄出。"
                ]
              }
            ]
          }
...
情境：課程推廣小組討論 8 月研習活動準備事項。
原始資料：今天討論 AI 協作新思維研習，日期維持 8/21 上午，地點在八甲校區國際會議廳。報名頁由教務處在 7/25 前完成，宣傳圖由設計組下週三前提供。各系所行政同仁至少派一人參加，講師資料與課程大綱要在 8/5 前寄給各單位。場地設備要確認投影、麥克風和 Wi-Fi。餐點先不用安排，茶水由總務處協助。
            ...
            sections: [
              {
                title: "建議操作",
                items: [
                  "先要求 ChatGPT 把內容整理成 Word 文件架構，再決定是否輸出成檔案。",
                  "若有 Word、文件或相關技能，可以要求產出 .docx；若沒有，就先輸出可貼到 Word 的格式。",
                  "正式送出前仍要人工確認會議名稱、日期、出席人員、決議與待辦期限。"
                ]
              },
              {
                title: "追加指令範例",
                items: [
                  "請把以上會議紀錄整理成 Word 文件格式，包含封面標題、會議資訊表、摘要、決議事項、待辦事項表格與待確認清單。若可產生檔案，請輸出 .docx；若無法產生檔案，請提供可直接貼到 Word 的排版內容。"
                ]
              }
            ]
          }
...
情境：整理研習報名與出席資料，想了解報名狀況與是否有異常。
原始資料：
單位,姓名,是否報名,是否出席,餐食需求,備註
教務處,王小美,是,是,無,
學務處,陳志明,是,否,素食,臨時公務
總務處,林佳蓉,是,是,無,
研發處,黃建宏,否,否,無,
圖書館,張雅婷,是,是,素食,
國際處,李承恩,是,是,無,重複報名一次
...
5. 需要人工確認的地方`,
        detailGroups: [
          {
            title: "進階追問：Google 表單與 Excel",
            rows: [
              [
                "Google 表單",
                "先產生題目、選項與必填欄位；若帳號可連接 Google Forms 或雲端工具，再建立表單草稿。",
                "請把這份需求設計成 Google 表單題目清單，包含題型、選項、是否必填與說明文字。若可連接 Google 表單，請只建立草稿並列出需要我確認的設定。"
              ],
              [
                "Excel 檔案",
                "可要求建立欄位、統計表、樞紐分析建議與圖表建議；若有 Excel 或表格技能，再請它產生 .xlsx。",
                "請建立一份 Excel 工作簿架構，包含原始資料、清理檢查、統計摘要與圖表建議四個工作表。若可產生檔案，請輸出 .xlsx。"
              ]
            ],
            headers: ["需求", "建議做法", "追加指令範例"]
          }
        ]
      },
      {
...
情境：研習結束後，要向主管回報活動成果。
原始資料：AI 協作新思維研習於 8/21 上午舉辦，地點在八甲校區國際會議廳，共 86 人報名、72 人出席。課程內容包含 ChatGPT 基礎概念、提示詞練習、資料整理、簡報與公告改寫、資安提醒。問卷平均滿意度 4.6 分，學員最喜歡實作練習，但希望未來增加 Excel 與會議紀錄案例。後續建議建立校內提示詞範本庫，並安排進階班。
...
5. 主管最需要先看到的摘要`,
        detailGroups: [
          {
            title: "進階追問：製作 PowerPoint",
            sections: [
              {
                title: "建議操作",
                items: [
                  "先確認簡報目的、對象、頁數、比例、視覺風格與是否需要講者備註。",
                  "若有簡報、PowerPoint 或相關 Skills，可輸出 .pptx；若沒有，就先產出每頁標題、重點、版面建議與講稿。",
                  "使用 Skills 時，可在輸入框用 / 叫出可用技能，再選擇簡報或文件產出類技能。"
                ]
              },
              {
                title: "追加指令範例",
                items: [
                  "請把以上內容製作成 16:9 PowerPoint 簡報，頁數 8 頁，風格專業簡潔。每頁包含標題、3 個重點、版面建議、講者備註。若可使用簡報 Skill，請產生 .pptx；若無法產生檔案，請先輸出完整投影片腳本。"
                ]
              }
            ]
          }
...
情境：比較新版與舊版研習報名說明，找出需要公告給學員的差異。
原始資料：
舊版：研習時間為 8/20 09:00-12:00，地點為第二校區會議室，報名截止日為 8/10，參加者需自備筆電。
新版：研習時間改為 8/21 09:00-12:30，地點改為八甲校區國際會議廳，報名截止日延後至 8/15，參加者仍需自備筆電，並請事先確認可登入 ChatGPT 帳號。
...
情境：設計一堂 3 小時的 ChatGPT 入門研習，對象包含行政人員與教師，約 80 人。
原始資料：學員多數是初學者，需要先認識 ChatGPT 可以做什麼，再練習公告改寫、會議紀錄整理、表格資料分析、簡報大綱與資安查核。課程希望保留 30 分鐘實作與 10 分鐘 Q&A，整體風格要簡單、清楚、能立即上手。
        ...
        detailGroups: [
          {
            title: "進階追問：指定尺寸海報",
            rows: [
              ["尺寸比例", "A4 直式、16:9、1080 x 1350、1080 x 1080 或 1920 x 1080。"],
              ["用途", "列印、簡報封面、社群貼文、活動頁首或校內電子看板。"],
              ["內容限制", "主標題、副標、時間、地點、對象、報名方式、主辦單位與聯絡方式。"],
              ["設計提醒", "若要列印，請要求高解析度與留白；若要社群發布，請要求文字少、重點明確。"]
            ],
            headers: ["設定項目", "可填內容"]
          },
          {
            ...
            sections: [
              {
                title: "海報提示詞",
                items: [
                  "請根據以上活動內容設計一張海報。尺寸為 [A4 直式 / 1080 x 1350 / 16:9]，用途是 [列印 / 社群貼文 / 簡報封面]，風格為 [專業、清楚、校園感]。請提供主標題、副標、重點文案、版面配置、色彩建議與 AI 圖片生成描述。若可使用設計工具或建立圖像功能，請先產生預覽草稿。"
                ]
              }
            ]
          }
      ...
      },
      {
        ...
        material: `
          ...
          <span class="material-link-stack">
            ...
            <a href="./assets/docs/fruit-stand-work/fruit-stand-web-analytics.csv" target="_blank" rel="noopener noreferrer">網站瀏覽紀錄 CSV</a>
